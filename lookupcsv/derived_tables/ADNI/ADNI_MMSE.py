...
with open(r"C:\Users\ngoth\OneDrive - flsouthern.edu\Research\AI4AD\ncomms2022\lookupcsv\raw_tables\ADNI\ADNI_MMSE.csv") as csvfile:
    reader = csv.DictReader(csvfile)
    for row in reader:
        content = {}
        for column in column_names:
            if column in ['PHASE', 'ID', 'RID', 'SITEID', 'VISCODE', 'VISCODE2', 'USERDATE', 'USERDATE2', 'VISDATE']:
                ...
                content[column] = [row['WORD1'], row['WORD2'], row['WORD3'], row['MMTRIALS']]
                ...
                content[column] = [row['WORD1DL'], row['WORD2DL'], row['WORD3DL']]
                content[column] = sum([int(a) - 1 for a in content[column] if a in ['1', '2'] ])  # correct is 0, incorrect is 1 for each item, the higher the worse
...
with open(r'C:\Users\ngoth\OneDrive - flsouthern.edu\Research\AI4AD\ncomms2022\lookupcsv\derived_tables\ADNI\ADNI_MMSE.csv', 'w') as csvfile:
